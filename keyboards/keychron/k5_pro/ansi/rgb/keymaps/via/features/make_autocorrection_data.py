...
try:
  ...
  CORRECT_WORDS = set(['apparent', 'association', 'available', 'classification',
...
try:
  with open('/usr/share/dict/french', 'r') as f:
    for l in f:
      CORRECT_WORDS.add(l.strip().lower())
except Exception as e:
  print("Warning: Could not open /usr/share/dict/french. Run -> apt install wfrench")
  print(e)
...
def parse_file_lines(file_name: str) -> Iterator[Tuple[int, str, str]]:
  ...
  for line in open(file_name, 'rt'):
    ...
    line = line.replace('é', '^0')
    line = line.replace('à', '^1')
    line = line.replace('è', '^2')
    line = line.replace('ù', '^3')
    line = line.replace('â', '^4')
    line = line.replace('ê', '^5')
    line = line.replace('î', '^6')
    line = line.replace('ô', '^7')
    line = line.replace('û', '^8')
    line = line.replace('ç', '^9')
...
def check_typo_against_dictionary(typo: str) -> None:
  ...
  if typo.startswith(':') and typo.endswith(':'):
    if typo[1:-1] in CORRECT_WORDS:
      print(f'Warning: Typo "{typo}" is a correctly spelled '
            'dictionary word.')
  elif typo.startswith(':') and not typo.endswith(':'):
    for word in CORRECT_WORDS:
      if word.startswith(typo[1:]):
        print(f'Warning: Typo "{typo}" would falsely trigger '
