...
void send_windows_accent(uint8_t keycode) {
...
void send_accent(uint8_t keycode) {
  #ifdef WINDOWS_ALT_ACCENT
  if (get_unicode_input_mode() == UC_WINC) {
    send_windows_accent(keycode);
    return ;
  }
  #endif
...
bool process_autocorrection(uint16_t keycode, keyrecord_t* record) {
  ...
  for (int i = typo_buffer_size - 1; i >= 0; --i) {
    ...
    if (code & 128) { // A typo was found! Apply autocorrection.
      char const * const replacement = (char const*)(autocorrection_data + state + 1);

      if (!autocorrect_enabled_accent && strchr(replacement, '^') != NULL) {
        return true;
      }
      ...
      for (int i = 0; i < replacement[i]; ++i) {
        if (replacement[i] == '^') {
          ++i;
          send_accent(replacement[i]);
          continue;
        }
        send_char(replacement[i]);
      }
